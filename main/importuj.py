...
def importuj_klientow(cur):
    ...
    with open(nazwa, "r") as plik:
        ...
        for line in reader:
            lista.append(line)
        cur.executemany("""INSERT INTO klienci VALUES (%s, %s, %s)""", lista)
    return
...
def importuj_odczyty(cur):
    ...
    with open(nazwa, "r") as plik:
        ...
        for line in reader:
            lista.append(line[1:])
        cur.executemany("""INSERT INTO odczyty (Wartosc, Data_odczytu, Miejscowosc, Ulica, Nr_domu, Nr_mieszkania, ID_klienta)
